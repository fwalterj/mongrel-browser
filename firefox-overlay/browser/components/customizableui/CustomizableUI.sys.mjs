 ...
 */
// Mongrel: bumped from upstream 24 → 25 to introduce a migration that strips
// firefox-view-button from existing profiles' toolbar placements (see
// `currentVersion < 25` block below). The button is also removed from new
// profiles' default placements for AREA_NAVBAR vertical mode and AREA_TABSTRIP.
var kVersion = 25;
...
var CustomizableUIInternal = {
  ...
  initialize() {
    ...
    // Mongrel: Downloads is intentionally NOT pinned to the nav-bar by default.
    // Users can add it back via Customize Toolbar; the downloads panel is still
    // reachable from the menu and via Cmd-J. Reasoning: the nav-bar should be
    // editorial (back/fwd/url/account), not a status surface.
    ...
    this.registerArea(
      CustomizableUI.AREA_NAVBAR,
      {
        ...
        // Mongrel: firefox-view-button removed from vertical-tabs default
        // navbar placement. Firefox View is a Mozilla-branded surface that
        // breaks Mongrel immersion; users can re-pin via Customize Toolbar.
    ...
    this.registerArea(
      ...
      {
        type: CustomizableUI.TYPE_TOOLBAR,
        // Mongrel: firefox-view-button removed from default tab strip — see
        // kVersion bump and firefox-view-button migration below.
        defaultPlacements: [
          "tabbrowser-tabs",
          "new-tab-button",
          "alltabs-button",
  ...
  updateForNewVersion() {
    ...
    // Mongrel: the upstream Firefox migration here was "if currentVersion < 11,
    // forcibly move the downloads-button into the navbar". We invert that:
    // for any profile that hasn't been Mongrel-migrated yet, REMOVE
    // downloads-button from the navbar so the user starts with a clean,
    // editorial toolbar. They can re-pin via Customize Toolbar if they want it.
    if (currentVersion < 11) {
      for (let placements of Object.values(gSavedState.placements)) {
        let existingIndex = placements.indexOf("downloads-button");
        if (existingIndex != -1) {
          placements.splice(existingIndex, 1);
          break; // It can only be in 1 place.
        }
      }
    }
    ...
    // Mongrel: upstream "Add firefox-view if not present" (currentVersion < 18)
    // intentionally NEUTRALIZED. Firefox View is a Mozilla-branded surface and
    // is not pinned by default in Mongrel; the strip migration in
    // `currentVersion < 25` removes it from any profiles where it survived.
    // Keeping the version-gate here as a no-op preserves migration ordering.
    ...
    // Mongrel: strip firefox-view-button from any saved placements. Firefox
    // View is a Mozilla-branded surface that breaks Mongrel's immersion;
    // users who want it can re-pin via Customize Toolbar. This mirrors the
    // earlier downloads-button strip migration.
    if (currentVersion < 25) {
      for (let placements of Object.values(gSavedState.placements)) {
        let idx = placements.indexOf("firefox-view-button");
        if (idx != -1) {
