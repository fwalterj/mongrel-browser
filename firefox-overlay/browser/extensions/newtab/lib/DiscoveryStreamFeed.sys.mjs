...
export class DiscoveryStreamFeed {
  ...
  setupPrefs(isStartup = false) {
    const experimentMetadata = lazy.NimbusFeatures.newtab.getEnrollmentMetadata();

    let utmSource = "newtab";
  ...
  async loadSpocs(sendUpdate, isStartup) {
    // Mongrel Phase 7: Pocket/spoc sponsorship is disabled; bail out before any cache or
    // network access.  showSponsoredStories already guards the fetch block below, but this
    // exit makes the intent explicit and cheaper.
