...
// Mongrel: when our start page is the product, route every load of
// `about:home` and `about:newtab` to the Mongrel chrome page that's already
// used as `browser.startup.homepage`. This catches the cases the homepage
// pref alone can't fix:
//   * sessionstore restoring an old `about:home`/`about:newtab` tab
//   * extensions or scripts navigating to about:home explicitly
//   * the Home button after a stale homepage has been migrated away
//   * any code path that bypasses `browser.startup.homepage`
// Default-on; flip `mongrel.startpage.replace_about_pages` to false in
// about:config to fall back to upstream Activity Stream.
XPCOMUtils.defineLazyPreferenceGetter(
  lazy,
  "MONGREL_REPLACE_ABOUT_PAGES",
  "mongrel.startpage.replace_about_pages",
  true
);
const MONGREL_START_PAGE_URL = "chrome://browser/content/newtab.html";
...
class BaseAboutNewTabRedirector {
   ...
   */
  get defaultURL() {
    // Mongrel: short-circuit Activity Stream so about:home / about:newtab
    // both resolve to the Mongrel chrome start page. See the pref-getter
    // comment at the top of this file.
    if (lazy.MONGREL_REPLACE_ABOUT_PAGES) {
      return MONGREL_START_PAGE_URL;
    }
  ...
  getURIFlags() {
    // Mongrel: when about:home / about:newtab resolve to a chrome:// page
    // (see `defaultURL` above), we drop URI_MUST_LOAD_IN_CHILD and
    // URI_CAN_LOAD_IN_PRIVILEGEDABOUT_PROCESS. Chrome content can only load
    // in the parent process; without these flags Gecko routes the about:
    // navigation to the parent where the chrome target resolves natively
    // (the same process that already loads `browser.startup.homepage` on
    // boot). URI_SAFE_FOR_UNTRUSTED_CONTENT stays so untrusted content can
    // still navigate to about:home / about:newtab — those navigations just
    // land us on the Mongrel chrome page like any other.
    if (lazy.MONGREL_REPLACE_ABOUT_PAGES) {
      return (
...
export class AboutNewTabRedirectorParent extends BaseAboutNewTabRedirector {
  ...
  newChannel(uri, loadInfo) {
    ...
    // Mongrel: in our mode, both about:home and about:newtab resolve to the
    // chrome start page regardless of `browser.newtabpage.enabled`. The
    // arkenfox baseline (mongrel-arkenfox.js) doesn't disable newtabpage,
    // but a power user might — and we still want them on Mongrel rather
    // than blanktab.html.
    if (lazy.MONGREL_REPLACE_ABOUT_PAGES) {
      if (
    ...
    // Mongrel: when we're routing to the chrome start page, the addon-init
    // suspension dance doesn't apply (the chrome page doesn't depend on the
    // newtab built-in addon), so skip it. Suspending here would make the
    // boot tab hang waiting for an addon we're not using.
    if (
      !this.#addonInitialized &&
      !(
        lazy.MONGREL_REPLACE_ABOUT_PAGES &&
        chromeURI.spec === MONGREL_START_PAGE_URL
      )
    ) {
...
export class AboutNewTabRedirectorChild extends BaseAboutNewTabRedirector {
  newChannel(uri, loadInfo) {
    ...
    // Mongrel: in our mode the parent's URI flags strip
    // URI_CAN_LOAD_IN_PRIVILEGEDABOUT_PROCESS, so the navigation should
    // never actually arrive in this child. If it somehow does (a tab in an
    // older privilegedabout process from before Mongrel mode was active,
    // for instance), refuse to serve the upstream Activity Stream — the
    // parent will retry the load in the right process and we'll surface
    // the Mongrel chrome page. Disqualifying the cache here also stops
    // a stale about:home cache from being served.
    if (lazy.MONGREL_REPLACE_ABOUT_PAGES) {
      AboutHomeStartupCacheChild.disqualifyCache();
      throw Components.Exception(
        "Mongrel start page replaces about:home/about:newtab in the parent process.",
        Cr.NS_ERROR_UNEXPECTED
      );
    }
