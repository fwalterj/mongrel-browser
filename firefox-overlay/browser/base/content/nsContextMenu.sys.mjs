...
export class nsContextMenu {
  ...
  initItems(aXulMenu) {
    ...
    this.mongrelReorderPriorityContextItems(aXulMenu);
  ...
  initImageItems() {
    ...
    this.showItem(
      "context-saveimage-as-png",
      this.onLoadedImage && this.onImage && !this.onCanvas
    );
  ...
  /**
   * Mongrel: export the clicked <img> as PNG via an in-content canvas (decoded
   * pixels). Fails for cross-origin images without CORS (tainted canvas).
   */
  saveImageAsPNG() {
    let isPrivate = lazy.PrivateBrowsingUtils.isBrowserPrivate(this.browser);
    let referrerInfo = this.contentData.referrerInfo;
    ...
    if (this.mediaURL) {
      try {
        ...
        let url = uri.QueryInterface(Ci.nsIURL);
        if (url.fileBaseName) {
          base = decodeURI(url.fileBaseName);
        }
      } catch (e) {}
    }
    let aName = `${base}.png`;

    this.actor.saveImageAsPNG(this.targetIdentifier).then(dataURL => {
      if (!dataURL) {
        Services.prompt.alert(
          this.window,
          this.window.document.title,
          "Could not save as PNG (cross-origin image or unsupported)."
        );
        return;
      }
      this.window.internalSave(
  ...
  /**
   * Mongrel: move the most relevant save/view actions directly under the
   * navigation block so image/video/audio targets feel “scoped” to the hit.
   */
  mongrelReorderPriorityContextItems(popup) {
    const sepNav = this.document.getElementById("context-sep-navigation");
    if (!sepNav || sepNav.hidden) {
      return;
    }
    let ref = sepNav.nextSibling;
    const moveIfVisible = id => {
      let el = this.document.getElementById(id);
      if (!el || el.hidden) {
        return;
      }
      popup.insertBefore(el, ref);
      ref = el.nextSibling;
    };
    ...
    } else if ((this.onLoadedImage && this.onImage) || this.onCanvas) {
      for (let id of [
        "context-reloadimage",
        "context-viewimage",
        "context-saveimage",
